...
from bias.GenerateSpikeProtein import generateSpikes
...
def drawGrids(distanceOfLinesInGraph, draw, img, graphGenomeAddress):
    ...
    yStart = 0
    yEnd = height
    for y in range(0, width, distanceOfLinesInGraph):
        line = ((xStart, y), (xEnd, y))
        draw.line(line, fill=128, width=5)
    for x in range(0, height, distanceOfLinesInGraph):
...
def drawGraphGenome(inFile):
    ...
    processSequences(sequenceList, rGenome, repeatList, nucleotideDictLists, threshold, distanceOfLinesInGraph)


def processSequences(seqList, rGenome, repetitionList, nucleotideDictLists, threshold, distanceOfLinesInGraph):
    """
...
def saveDifferenceArea(repetitionList, countOfDeference, differenceAmount, yAxes):
    ...
    # information that is needed for drawing the graph,
    # img = Image.new("RGB", (2000, 2000), (255, 255, 255))
    img = Image.new(mode='RGB', size=(height, width), color=255)
